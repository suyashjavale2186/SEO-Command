...
**Purpose.** This reference is the operating memory for `internal-link-actions` when the exact job is to **improve a hub page by adding meaningful connections to its most useful spokes**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Analyst frame

The unit of work is **hub strengthening**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Procedure

- choose spokes by reader task and topical fit [Evidence Tier: Tier 3 — Industry observation]
- place links near the relevant explanation [Evidence Tier: Tier 3 — Industry observation]
- remove dead or circular links [Evidence Tier: Tier 3 — Industry observation]
- check that the hub still reads naturally [Evidence Tier: Tier 3 — Industry observation]
...
A cybersecurity hub about endpoint protection should link from its explanation of detection methods to specific product-neutral guides on EDR, managed response, and incident triage; the links should help the reader choose the next level of detail. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Breakpoints

['stuffing every spoke into the hub', 'using identical anchor text everywhere', 'creating links with no explanatory context'] [Evidence Tier: Tier 3 — Industry observation]
