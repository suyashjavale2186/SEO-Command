...
**Purpose.** This reference is the operating memory for `internal-link-actions` when the exact job is to **select among adding, removing, moving, or rewriting an internal link**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Definition of done

The unit of work is **link action types**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Execution path

- add when a missing relationship blocks a useful next step [Evidence Tier: Tier 3 — Industry observation]
- move when placement obscures relevance [Evidence Tier: Tier 3 — Industry observation]
- rewrite anchor when it misdescribes destination [Evidence Tier: Tier 3 — Industry observation]
- remove when redundant or misleading [Evidence Tier: Tier 3 — Industry observation]
...
A comparison page links to a pricing page only from a footer. Moving the link into the section that discusses total cost can improve discoverability and context without adding another copy of the same relationship. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Common traps

['adding links when a move would solve the issue', 'anchors that promise information not present', 'removing useful links solely to reduce count'] [Evidence Tier: Tier 3 — Industry observation]
