...
**Purpose.** This reference is the operating memory for `content-updates` when the exact job is to **remove content when it no longer serves a valid user or business purpose**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Use conditions

The unit of work is **delete**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Investigation route

- confirm there is no important distinct intent [Evidence Tier: Tier 3 — Industry observation]
- inspect inbound links and business references [Evidence Tier: Tier 3 — Industry observation]
- identify a replacement only when it truly serves the same task [Evidence Tier: Tier 3 — Industry observation]
- record removal and validation [Evidence Tier: Tier 3 — Industry observation]
...
A discontinued event microsite with no evergreen information may be removed after checking links and business records; a permanent resource that merely lost traffic should not be deleted on that signal alone. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Invalid patterns

['deleting from traffic alone', 'redirecting everything to the homepage', 'removing compliance or historical records without authorization'] [Evidence Tier: Tier 3 — Industry observation]
