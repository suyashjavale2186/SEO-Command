...
**Purpose.** This reference is the operating memory for `content-updates` when the exact job is to **choose the appropriate maintenance action from observed page conditions**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Definition of done

The unit of work is **update decision tree**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Execution path

- check intent continuity [Evidence Tier: Tier 3 — Industry observation]
- check unique value [Evidence Tier: Tier 3 — Industry observation]
- check technical health [Evidence Tier: Tier 3 — Industry observation]
- check overlap with other URLs [Evidence Tier: Tier 3 — Industry observation]
- select refresh, rewrite, consolidate, redirect, or delete [Evidence Tier: Tier 1 — Officially documented]
...
An outdated comparison page that still receives qualified traffic and has a distinct intent is a refresh candidate; a near-duplicate page whose best material is covered elsewhere may be a consolidation candidate. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Common traps

['choosing an action before diagnosis', 'deleting valuable URLs because traffic is small', 'redirecting to irrelevant destinations'] [Evidence Tier: Tier 3 — Industry observation]
