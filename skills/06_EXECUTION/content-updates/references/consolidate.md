...
**Purpose.** This reference is the operating memory for `content-updates` when the exact job is to **combine overlapping assets when one stronger page can serve their shared search job**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Analyst frame

The unit of work is **consolidate**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Procedure

- compare intent and unique evidence [Evidence Tier: Tier 3 — Industry observation]
- choose the canonical surviving asset based on user/business fit [Evidence Tier: Tier 1 — Officially documented]
- move useful material deliberately [Evidence Tier: Tier 3 — Industry observation]
- redirect or otherwise retire redundant URLs [Evidence Tier: Tier 1 — Officially documented]
- rebuild internal links [Evidence Tier: Tier 3 — Industry observation]
...
Three near-duplicate beginner guides about VAT registration can be consolidated into a single jurisdiction-specific guide when their search task and source basis are materially the same; distinct country rules should remain separated. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Breakpoints

['merging incompatible intents', 'discarding unique evidence', 'redirecting before the destination contains the promised information'] [Evidence Tier: Tier 3 — Industry observation]
