...
**Purpose.** This reference is the operating memory for `technical-fixes` when the exact job is to **verify that a technical SEO change produced the intended state in production**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Control object

The unit of work is **validation**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Diagnostic route

- check response behavior, rendered output, links, metadata, and representative URL classes [Evidence Tier: Tier 3 — Industry observation]
- compare before/after evidence [Evidence Tier: Tier 3 — Industry observation]
- monitor downstream Search Console or analytics signals when relevant [Evidence Tier: Tier 1 — Officially documented]
...
After changing a canonical template, validate source HTML and representative parameter variants before relying on Search Console's later observations; the validation record should identify any unexpected variant that still points elsewhere. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Abort conditions

['testing only staging', 'checking one URL class', 'calling a deployment successful without verifying the user-facing state'] [Evidence Tier: Tier 3 — Industry observation]
