---
name: serp-analysis
description: Inspect the live search results for a target query to understand intent, result formats, competitors, SERP features, volatility, and the information users are currently being offered.
---

# Serp Analysis

## Purpose
Inspect the live search results for a target query to understand intent, result formats, competitors, SERP features, volatility, and the information users are currently being offered.

## When to trigger this skill
When a recommendation depends on current search results, a target query, or a page that is expected to compete in organic search.

## Step-by-step workflow
1. Search the target query in the relevant market, language, device context, and location when known.
2. Inspect leading organic results and relevant SERP features; go deeper where the SERP is ambiguous or highly heterogeneous.
3. Record result type, apparent intent, unique value, authority/context, content format, and notable feature patterns.
4. Distinguish observed SERP patterns from causal ranking explanations.
5. Feed findings into intent-analysis, competitor-analysis, content-brief, and serp-feature-optimization.
...
Return: query context; SERP map; intent interpretation; competitor set; feature map; evidence; recommendation blocks.
