...
**Purpose.** This reference gives the `generative search` skill a concrete operating method for **generative search measurement**. Use it as an internal diagnostic and decision framework; it does not represent a hidden Google ranking formula. [Evidence Tier: Tier 4 — Hypothesis]


## Core principles


- Track generative visibility as an observable reporting problem: where the site appears, which pages are cited, for which queries, and with what context. [Evidence Tier: Tier 4 — Hypothesis]
...
Prefer first-party Search Console data where available and document gaps when third-party monitoring is used. Begin by documenting the baseline and the uncertainty around it. If the evidence is thin, the correct output may be a monitoring recommendation or a request for better data rather than a confident optimization plan. [Evidence Tier: Tier 2 — Strong empirical evidence]
