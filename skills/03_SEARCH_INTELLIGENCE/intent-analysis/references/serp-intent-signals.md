...
**Source basis:** Ported and filtered from: skills/keyword-deep-dive/references/intent-matching.md. The legacy structure is retained where it is operationally useful; unsupported Google-mechanism claims and arbitrary SEO targets are not carried forward. [Evidence Tier: Tier 3 — Industry observation]
...
The retained pattern should be applied to the real user and business context: Infer intent from result formats, titles, page types, commercial modules, recurring sections, and the consistency of results. The reference is a decision aid, not a ranking formula. Start with the observed situation, classify the underlying task or risk, select an intervention that directly addresses it, and define how the result will be validated. [Evidence Tier: Tier 4 — Hypothesis]
