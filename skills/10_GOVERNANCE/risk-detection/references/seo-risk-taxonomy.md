...
**Purpose.** This reference is the operating memory for `risk-detection` when the exact job is to **classify SEO risk by mechanism and consequence for consistent governance**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## What to capture

The unit of work is **seo risk taxonomy**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Field method

- separate technical, content, authority, measurement, compliance, and business risks [Evidence Tier: Tier 3 — Industry observation]
- record trigger, consequence, evidence, and mitigation [Evidence Tier: Tier 3 — Industry observation]
- distinguish policy risk from ranking uncertainty [Evidence Tier: Tier 3 — Industry observation]
...
A paid-link campaign is an authority-policy risk, while a broken hreflang deployment is a technical targeting risk; placing both in a single 'SEO risk' bucket hides the action needed. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Do-not-do list

['mixing probability and impact', 'treating hypotheses as penalties', 'using a single generic mitigation'] [Evidence Tier: Tier 3 — Industry observation]
