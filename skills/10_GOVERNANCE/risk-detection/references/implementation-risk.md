...
**Purpose.** This reference is the operating memory for `risk-detection` when the exact job is to **identify execution risks that can derail a technically valid SEO recommendation**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Definition of done

The unit of work is **implementation risk**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Execution path

- inspect dependency failure, rollback complexity, monitoring gaps, data corruption, and ownership ambiguity [Evidence Tier: Tier 3 — Industry observation]
- rank risk by consequence and detectability [Evidence Tier: Tier 3 — Industry observation]
- define a pre-release guardrail [Evidence Tier: Tier 3 — Industry observation]
...
A canonical template change affects thousands of product URLs; the implementation risk is not the canonical concept itself but the possibility that a template bug changes all representatives simultaneously. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Common traps

['calling every uncertainty an SEO penalty', 'ignoring rollback', 'shipping changes without validation ownership'] [Evidence Tier: Tier 3 — Industry observation]
