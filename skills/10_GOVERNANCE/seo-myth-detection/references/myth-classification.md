...
**Purpose.** This reference is the operating memory for `seo-myth-detection` when the exact job is to **classify an SEO claim by evidence status and remove false precision**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Analyst frame

The unit of work is **myth classification**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Procedure

- label documented guidance separately from observation and hypothesis [Evidence Tier: Tier 3 — Industry observation]
- identify the hidden assumption [Evidence Tier: Tier 3 — Industry observation]
- rewrite unsupported certainty into a testable proposition [Evidence Tier: Tier 3 — Industry observation]
...
A legacy document claims a fixed title character limit and keyword-density target. The myth audit replaces both with goals of relevance, clarity, differentiation, and natural terminology, while recording the original claims as unsupported rules. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Breakpoints

['preserving folklore because it is common', 'confusing correlation with policy', 'turning a heuristic into a requirement'] [Evidence Tier: Tier 3 — Industry observation]
