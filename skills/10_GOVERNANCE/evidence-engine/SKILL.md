...
1. Collect the claim or recommendation and separate observation, interpretation, causal claim, and action.
2. Prefer primary official documentation for Google-specific rules and capabilities; use reproducible studies for causal or quantitative assertions; use industry sources only for observed patterns; label hypotheses as hypotheses.
3. Check source date and scope. Reject sources that do not actually support the specific assertion.
4. Assign the weakest defensible tier. Record what evidence would be needed to move the claim to a stronger tier.
5. Pass the classified evidence into the calling skill. Block recommendations that cannot state evidence status.
...
Return an evidence ledger: Claim, Source, Tier, What the source actually establishes, What remains inference, Confidence, and Evidence gap. Then emit the required recommendation block for any action derived from the ledger.
...
The evidence engine is the gatekeeper: a recommendation without an evidence classification is incomplete.
