...
**Purpose.** This reference is the operating memory for `conversion-model` when the exact job is to **classify business outcomes before selecting measurement**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Decision boundary

The unit of work is **conversion taxonomy**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Operator sequence

- separate macro conversions from assisting and diagnostic events [Evidence Tier: Tier 3 — Industry observation]
- classify by commitment level and business value [Evidence Tier: Tier 3 — Industry observation]
- map categories to the customer journey and operating owner [Evidence Tier: Tier 3 — Industry observation]
...
For a subscription analytics product, a trial signup, activated workspace, sales-qualified account, paid conversion, expansion, and renewal are different conversion types; treating every button click as the same outcome would obscure the commercial hierarchy. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Failure signatures

['calling every interaction a conversion', 'mixing lead-stage events with revenue outcomes', 'creating categories that the CRM cannot actually record'] [Evidence Tier: Tier 3 — Industry observation]
