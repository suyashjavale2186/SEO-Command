...
**Purpose.** This reference is the operating memory for `seo-roadmap` when the exact job is to **translate the roadmap into a reviewable planning cycle without treating dates as guarantees**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Definition of done

The unit of work is **quarterly planning**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Execution path

- choose a small set of strategic themes [Evidence Tier: Tier 3 — Industry observation]
- define evidence gates for continuation or reprioritization [Evidence Tier: Tier 3 — Industry observation]
- carry unresolved experiments and learning into the next planning cycle [Evidence Tier: Tier 3 — Industry observation]
...
A quarter focused on non-brand commercial discovery may begin with category architecture and Search Console baselines, then shift resources toward pages whose evidence shows the strongest opportunity. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Common traps

['equating completed tickets with strategic progress', 'locking the plan despite new evidence', 'using quarter-end traffic as the only success test'] [Evidence Tier: Tier 3 — Industry observation]
