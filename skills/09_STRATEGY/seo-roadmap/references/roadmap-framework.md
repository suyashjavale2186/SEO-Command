...
**Purpose.** This reference is the operating memory for `seo-roadmap` when the exact job is to **turn prioritized SEO opportunities into a coherent delivery plan**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Use conditions

The unit of work is **roadmap framework**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Investigation route

- group work by strategic outcome rather than by tool feature [Evidence Tier: Tier 3 — Industry observation]
- show prerequisites, owners, success evidence, and review points [Evidence Tier: Tier 3 — Industry observation]
- reserve space for measurement and iteration [Evidence Tier: Tier 3 — Industry observation]
...
A retailer's roadmap can combine indexation repair, high-value category improvements, and content refreshes into one program whose sequencing follows business impact and implementation dependency rather than producing unrelated tickets. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Invalid patterns

['filling the roadmap with low-impact tasks', 'promising traffic outcomes as deadlines', 'omitting post-release validation'] [Evidence Tier: Tier 3 — Industry observation]
