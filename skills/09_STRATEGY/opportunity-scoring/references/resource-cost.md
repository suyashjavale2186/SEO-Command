...
**Purpose.** This reference is the operating memory for `opportunity-scoring` when the exact job is to **estimate the true resource burden of an SEO opportunity**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Use conditions

The unit of work is **resource cost**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Investigation route

- include research, implementation, review, publishing, and measurement work [Evidence Tier: Tier 3 — Industry observation]
- identify specialist dependencies [Evidence Tier: Tier 3 — Industry observation]
- account for opportunity cost when teams are constrained [Evidence Tier: Tier 3 — Industry observation]
...
A content refresh that requires legal review, product data extraction, design support, and engineering changes may be more expensive than its word count suggests; the opportunity model should reflect those real dependencies. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Invalid patterns

['counting only writing time', 'ignoring stakeholder approvals', 'using a generic effort label without an owner'] [Evidence Tier: Tier 3 — Industry observation]
