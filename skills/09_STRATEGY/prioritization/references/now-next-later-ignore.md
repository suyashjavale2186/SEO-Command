...
**Purpose.** This reference is the operating memory for `prioritization` when the exact job is to **turn scored work into an operational queue**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Decision boundary

The unit of work is **now next later ignore**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Operator sequence

- NOW contains urgent/high-leverage work with prerequisites ready [Evidence Tier: Tier 3 — Industry observation]
- NEXT contains validated work that follows current blockers [Evidence Tier: Tier 3 — Industry observation]
- LATER contains lower-confidence or dependency-bound work [Evidence Tier: Tier 3 — Industry observation]
- IGNORE contains ideas that do not justify their cost or evidence [Evidence Tier: Tier 3 — Industry observation]
...
A broken sitemap reference belongs in NOW when it affects important URLs; a speculative markup tweak with no demonstrated business or search benefit may belong in IGNORE until stronger evidence appears. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Failure signatures

['using buckets as arbitrary labels', 'keeping attractive ideas out of IGNORE', 'moving everything into NOW to create urgency'] [Evidence Tier: Tier 3 — Industry observation]
