...
**Purpose.** This reference is the operating memory for `technical-seo-audit` when the exact job is to **resolve conflicting URL representatives**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Use conditions

The unit of work is **canonicalization**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Investigation route

- compare canonical tags, redirects, internal links, sitemap inclusion, and content similarity [Evidence Tier: Tier 1 — Officially documented]
- identify the intended representative for each duplicate set [Evidence Tier: Tier 3 — Industry observation]
- remove conflicting signals instead of layering additional hints [Evidence Tier: Tier 3 — Industry observation]
...
A faceted catalog creates `/chairs?color=black` beside `/chairs`; if the filtered URL has no distinct search purpose, internal links and sitemap references should favor the representative catalog URL while tracking parameters are handled separately. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Invalid patterns

['canonical to materially different content', 'using canonicalization to hide navigation defects', "assuming Google's chosen canonical must match the declared one"] [Evidence Tier: Tier 3 — Industry observation]
