...
**Purpose.** This reference is the operating memory for `cannibalization` when the exact job is to **distinguish genuine intent overlap from harmless query variation**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## What to capture

The unit of work is **intent overlap**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Field method

- compare target queries, SERP result types, page promises, and conversions [Evidence Tier: Tier 3 — Industry observation]
- look for pages that answer the same task with different wording [Evidence Tier: Tier 3 — Industry observation]
- treat shared vocabulary alone as insufficient evidence [Evidence Tier: Tier 3 — Industry observation]
...
A software company has `/best-crm-for-startups` and `/crm-for-startups`; if both target evaluation intent and surface the same shortlist, the overlap is material, whereas a separate `/crm-implementation-guide` serves a different task. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Do-not-do list

['merging pages because they share words', 'using rank alone as proof of cannibalization', 'ignoring different funnel stages'] [Evidence Tier: Tier 3 — Industry observation]
