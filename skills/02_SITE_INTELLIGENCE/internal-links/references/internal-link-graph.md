...
**Source basis:** Ported and filtered from: skills/topic-cluster-planning/references/topic-cluster-strategy.md, skills/topic-cluster-planning/references/first-link-weight-evidence.md. The legacy structure is retained where it is operationally useful; unsupported Google-mechanism claims and arbitrary SEO targets are not carried forward. [Evidence Tier: Tier 3 — Industry observation]
...
The retained pattern should be applied to the real user and business context: Analyze source-destination relationships across the site to identify weak hubs, disconnected pages, and redundant link paths. The reference is a decision aid, not a ranking formula. Start with the observed situation, classify the underlying task or risk, select an intervention that directly addresses it, and define how the result will be validated. [Evidence Tier: Tier 4 — Hypothesis]
...
Suppose A graph can show that several important pages rely on a single thin hub; strengthen the hub or connect the important pages directly when that improves user navigation. Begin by proving the symptom exists and identifying the page/query/business role involved. If the evidence points to a content problem, add useful information or fix the intent mismatch; if it points to technical access or indexation, fix the underlying technical state first. Do not use a content rewrite to treat a problem whose evidence points elsewhere. [Evidence Tier: Tier 2 — Strong empirical evidence]
