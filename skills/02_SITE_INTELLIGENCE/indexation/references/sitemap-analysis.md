...
Compare sitemap URLs with canonical URLs, important page sets, HTTP status, and observed indexing so the sitemap reflects the site's intended search inventory. Begin by documenting the baseline and the uncertainty around it. If the evidence is thin, the correct output may be a monitoring recommendation or a request for better data rather than a confident optimization plan. [Evidence Tier: Tier 2 — Strong empirical evidence]
