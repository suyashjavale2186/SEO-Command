---
name: indexation
description: Diagnose whether important URLs are eligible, discovered, crawled, and represented in search as intended, with clear distinctions between technical causes and content clustering.
---

# Indexation

## Purpose
Diagnose whether important URLs are eligible, discovered, crawled, and represented in search as intended, with clear distinctions between technical causes and content clustering.

## When to trigger this skill
When indexation coverage is unclear, pages are missing from search, canonical signals conflict, or Search Console reports unexpected URL states.

## Step-by-step workflow
1. Inventory target URLs and their intended search role.
2. Compare supplied/indexed data with crawlable status, HTTP response, noindex, canonical signals, sitemap inclusion, and duplicate relationships.
3. Use URL Inspection/Search Console evidence when available; do not infer indexed status solely from a site query.
4. Classify each issue: discoverability, crawlability, indexing eligibility, canonicalization, duplication, or content-quality clustering.
5. Recommend only fixes that address the observed class of issue and define a validation method.
...
Return: URL-state matrix; issue classes; evidence; remediation; validation plan; recommendation blocks.
