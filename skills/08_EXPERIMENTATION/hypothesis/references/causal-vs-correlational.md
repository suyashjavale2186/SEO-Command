...
**Purpose.** This reference is the operating memory for `hypothesis` when the exact job is to **separate observed co-movement from evidence that a change caused an SEO outcome**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Use conditions

The unit of work is **causal vs correlational**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Investigation route

- list competing explanations [Evidence Tier: Tier 3 — Industry observation]
- check whether the intervention changed while relevant confounders also changed [Evidence Tier: Tier 3 — Industry observation]
- downgrade causal language when the design cannot isolate the effect [Evidence Tier: Tier 3 — Industry observation]
...
A title rewrite coincides with more clicks, but an algorithm update and seasonal demand shift happened in the same period; the record can say the metrics moved after the change, not that the rewrite caused the entire increase. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Invalid patterns

['post hoc attribution', 'ignoring simultaneous changes', 'using correlation to describe a proven effect'] [Evidence Tier: Tier 3 — Industry observation]
