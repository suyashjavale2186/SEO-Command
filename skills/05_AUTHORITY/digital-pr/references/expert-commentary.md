...
**Purpose.** This reference is the operating memory for `digital-pr` when the exact job is to **turn genuine subject-matter expertise into usable media commentary without manufacturing credentials**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Decision boundary

The unit of work is **expert commentary**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Operator sequence

- select a real expert with documented role [Evidence Tier: Tier 3 — Industry observation]
- ask for a specific interpretation of the event or data [Evidence Tier: Tier 3 — Industry observation]
- capture caveats and boundaries [Evidence Tier: Tier 3 — Industry observation]
- quote accurately with approval [Evidence Tier: Tier 3 — Industry observation]
...
A security engineer can explain why a newly disclosed vulnerability changes deployment priorities based on observed product architecture; the commentary should stay within that expertise rather than adding invented predictions. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Failure signatures

['invented credentials', 'generic AI-written quotes', 'quotes that remove necessary caveats', 'using one expert outside their domain'] [Evidence Tier: Tier 3 — Industry observation]
