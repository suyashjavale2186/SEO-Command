...
description: Turn Search Console data into evidence about queries, pages, visibility changes, and search behavior while respecting the limits of aggregated and sampled search data.
---

# Search Console Analysis

## Purpose
Turn Search Console data into evidence about queries, pages, visibility changes, and search behavior while respecting the limits of aggregated and sampled search data.

## When to trigger this skill
When Search Console data is available or when diagnosing organic search changes.

## Step-by-step workflow
1. Define the business/question being investigated before selecting filters.
2. Analyze clicks, impressions, CTR, and average position by relevant page/query/country/device/search appearance dimensions when available.
3. Compare appropriate periods and control for seasonality, query mix, site changes, and reporting-method changes.
4. Look for page-query mismatches, emerging queries, declining queries, CTR opportunities, and query clusters tied to business outcomes.
5. State attribution and aggregation limits; do not infer a causal mechanism from a single metric movement.
6. Feed evidence into content-decay, opportunity-scoring, and experimentation.
...
Return: question; dataset/scope; observations; interpretation; limits; recommendation blocks.
