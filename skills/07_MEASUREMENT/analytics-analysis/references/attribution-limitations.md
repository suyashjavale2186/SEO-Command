...
**Purpose.** This reference is the operating memory for `analytics-analysis` when the exact job is to **show where analytics cannot prove that organic search caused an outcome**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Definition of done

The unit of work is **attribution limitations**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Execution path

- document attribution model and window [Evidence Tier: Tier 3 — Industry observation]
- look for channel interactions before claiming organic ownership [Evidence Tier: Tier 3 — Industry observation]
- label modeled or inferred revenue separately from observed conversions [Evidence Tier: Tier 3 — Industry observation]
...
A buyer first discovers a SaaS guide through search, returns by email, speaks with sales, and converts later; the report should explain how the chosen attribution model distributes value instead of presenting one channel as the sole cause. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Common traps

['claiming last-click revenue is incremental organic revenue', 'ignoring consent or tracking loss', 'treating attribution as causal proof'] [Evidence Tier: Tier 3 — Industry observation]
