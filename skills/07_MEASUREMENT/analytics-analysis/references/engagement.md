...
**Purpose.** This reference gives the `analytics analysis` skill a concrete operating method for **engagement analysis**. Use it as an internal diagnostic and decision framework; it does not represent a hidden Google ranking formula. [Evidence Tier: Tier 4 — Hypothesis]


## Core principles


- Use engagement metrics to diagnose whether landing-page traffic is useful, while recognizing measurement differences across implementations. [Evidence Tier: Tier 3 — Industry observation]
...
A short session can be success for a quick factual answer, so engagement should be interpreted by page intent. Begin by documenting the baseline and the uncertainty around it. If the evidence is thin, the correct output may be a monitoring recommendation or a request for better data rather than a confident optimization plan. [Evidence Tier: Tier 2 — Strong empirical evidence]
