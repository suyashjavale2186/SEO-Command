...
**Purpose.** This reference is the operating memory for `content-decay` when the exact job is to **measure whether an intervention restored the page's intended search and business function**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Use conditions

The unit of work is **recovery measurement**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Investigation route

- freeze the pre-change state and query set [Evidence Tier: Tier 3 — Industry observation]
- separate recovery of visibility from recovery of clicks and conversions [Evidence Tier: Tier 3 — Industry observation]
- document partial recovery where only some query clusters improve [Evidence Tier: Tier 3 — Industry observation]
...
An editorial site refreshes an evergreen guide after a sustained loss in impressions; the post-change readout separates recovery on the original topic from new traffic gained through adjacent queries so the team does not over-credit the intervention. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Invalid patterns

['using total site traffic as the recovery metric', 'changing several major variables at once without a record', 'declaring success because one day improved'] [Evidence Tier: Tier 3 — Industry observation]
