...
**Purpose.** This reference is the operating memory for `generative-search-visibility` when the exact job is to **track whether the brand or entity is mentioned in relevant generative answers**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Definition of done

The unit of work is **mention tracking**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Execution path

- define canonical brand and entity variants [Evidence Tier: Tier 1 — Officially documented]
- record favorable, neutral, incorrect, and missing mentions [Evidence Tier: Tier 3 — Industry observation]
- capture the surrounding claim so a bare name is not mistaken for meaningful visibility [Evidence Tier: Tier 3 — Industry observation]
...
A cybersecurity vendor appears in an answer about endpoint protection but is described as serving consumers; the tracker records both the mention and the incorrect association so the content team can address the underlying ambiguity. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Common traps

['counting every mention equally', 'ignoring incorrect entity context', 'treating model output as a stable factual database'] [Evidence Tier: Tier 3 — Industry observation]
