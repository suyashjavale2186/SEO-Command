...
**Purpose.** This reference is the operating memory for `generative-search-visibility` when the exact job is to **document what generative-search observation can and cannot establish**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Definition of done

The unit of work is **measurement limitations**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Execution path

- record query, market, model/surface, date, visible answer, citations, and source context [Evidence Tier: Tier 3 — Industry observation]
- distinguish observed absence from proof that the page was considered and rejected [Evidence Tier: Tier 3 — Industry observation]
- state coverage and sampling limitations [Evidence Tier: Tier 3 — Industry observation]
...
A daily prompt check may show a brand cited for 'enterprise backup comparison' but not for a neighboring query; that observation is useful, but it does not prove a stable model-wide visibility rate. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Common traps

['treating a small prompt sample as a universal share', 'assuming one AI surface represents another', 'claiming causal ranking effects from citation changes'] [Evidence Tier: Tier 3 — Industry observation]
