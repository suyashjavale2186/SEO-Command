...
**Purpose.** This reference is the operating memory for `generative-search-visibility` when the exact job is to **measure when site content is selected as a cited source in generative results**. Use it to decide what evidence to gather, how to act on the named artifact, and what would invalidate the conclusion. It is not a claim about an undisclosed ranking formula and it does not guarantee an outcome. [Evidence Tier: Tier 3 — Industry observation]

## Analyst frame

The unit of work is **citation tracking**, not “SEO” in general. Before opening a checklist, name the URL, dataset, query group, experiment, stakeholder decision, or outreach asset under review. Record the state that exists now and the state the business or user actually needs. The distinction matters: a technically clean implementation can still be the wrong action if it serves the wrong audience, market, or commercial objective. [Evidence Tier: Tier 3 — Industry observation]

## Procedure

- capture cited URL, query, answer passage, source role, and timestamp [Evidence Tier: Tier 3 — Industry observation]
- compare citation patterns across query classes [Evidence Tier: Tier 3 — Industry observation]
- review whether the cited page actually contains the evidence used in the answer [Evidence Tier: Tier 3 — Industry observation]
...
For a tax software company, a generative answer cites its current tax-credit guide but uses a stale threshold from another source; the audit records the citation and the answer error separately. The example is hypothetical unless the operator attaches an identifiable real source. Treat it as a pattern for reasoning, not evidence that the tactic caused a particular result. [Evidence Tier: Tier 3 — Industry observation]

## Breakpoints

['assuming citation means endorsement', 'counting duplicate citations as independent coverage', 'failing to inspect the cited passage'] [Evidence Tier: Tier 3 — Industry observation]
