...
**Purpose.** This reference gives the `generative search visibility` skill a concrete operating method for **governance contract**. Use it as an internal diagnostic and decision framework; it does not represent a hidden Google ranking formula. [Evidence Tier: Tier 4 — Hypothesis]


## Core principles


- Apply the plugin-wide evidence and safety contract to the local topic, with recommendation blocks, tier labels, uncertainty handling, and no fabricated experience. [Evidence Tier: Tier 4 — Hypothesis]
...
This reference exists so a skill can re-read its governing requirements without depending on another file at runtime. Begin by documenting the baseline and the uncertainty around it. If the evidence is thin, the correct output may be a monitoring recommendation or a request for better data rather than a confident optimization plan. [Evidence Tier: Tier 2 — Strong empirical evidence]
