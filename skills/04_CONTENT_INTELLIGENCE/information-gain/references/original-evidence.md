...
The retained pattern should be applied to the real user and business context: Classify evidence into first-party observations, measurements, experiments, records, examples, and expert explanations, then record provenance. The reference is a decision aid, not a ranking formula. Start with the observed situation, classify the underlying task or risk, select an intervention that directly addresses it, and define how the result will be validated. [Evidence Tier: Tier 4 — Hypothesis]
...
Suppose A screenshot without context is weak evidence; a reproducible process with dates, inputs, outputs, and limitations is stronger. Begin by proving the symptom exists and identifying the page/query/business role involved. If the evidence points to a content problem, add useful information or fix the intent mismatch; if it points to technical access or indexation, fix the underlying technical state first. Do not use a content rewrite to treat a problem whose evidence points elsewhere. [Evidence Tier: Tier 2 — Strong empirical evidence]
