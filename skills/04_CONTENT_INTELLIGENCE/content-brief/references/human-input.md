...
**Source basis:** Ported and filtered from: skills/content-brief/references/human-input-framework.md. The legacy structure is retained where it is operationally useful; unsupported Google-mechanism claims and arbitrary SEO targets are not carried forward. [Evidence Tier: Tier 3 — Industry observation]
...
The retained pattern should be applied to the real user and business context: Extract first-party knowledge from experts, operators, customers, sales teams, support teams, and proprietary data before drafting. The reference is a decision aid, not a ranking formula. Start with the observed situation, classify the underlying task or risk, select an intervention that directly addresses it, and define how the result will be validated. [Evidence Tier: Tier 4 — Hypothesis]
...
Suppose Ask for concrete events, numbers, failures, edge cases, and opinions; if the source cannot substantiate an experience, do not manufacture it. Begin by proving the symptom exists and identifying the page/query/business role involved. If the evidence points to a content problem, add useful information or fix the intent mismatch; if it points to technical access or indexation, fix the underlying technical state first. Do not use a content rewrite to treat a problem whose evidence points elsewhere. [Evidence Tier: Tier 2 — Strong empirical evidence]
