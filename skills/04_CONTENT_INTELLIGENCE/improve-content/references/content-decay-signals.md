...
- Systematically updating existing content to maintain and improve rankings -- because Google's freshness signals (confirmed via `bylineDateFreshness` in the API leak) penalize stale content and reward regularly updated pages. [Evidence Tier: Tier 3 — Industry observation]


- ## Why It Works [Evidence Tier: Tier 3 — Industry observation]


- The legacy source linked freshness changes to inferred API fields. Those fields are not a public Google ranking specification, so treat that mechanism as a hypothesis. The actionable lesson is narrower: when facts, prices, products, rules, or other time-sensitive information materially change, stale content should be reviewed and updated. [Evidence Tier: Tier 4 — Hypothesis]


- - Existing pages already have authority (backlinks, indexed history) [Evidence Tier: Tier 3 — Industry observation]


- - Updates trigger re-crawling and re-evaluation [Evidence Tier: Tier 3 — Industry observation]
...
The retained pattern should be applied to the real user and business context: Detect sustained deterioration through impressions, clicks, CTR, query coverage, position patterns, competitive changes, and content staleness. The reference is a decision aid, not a ranking formula. Start with the observed situation, classify the underlying task or risk, select an intervention that directly addresses it, and define how the result will be validated. [Evidence Tier: Tier 4 — Hypothesis]
...
Suppose Avoid labeling decay from one noisy metric or one anomalous period. Begin by proving the symptom exists and identifying the page/query/business role involved. If the evidence points to a content problem, add useful information or fix the intent mismatch; if it points to technical access or indexation, fix the underlying technical state first. Do not use a content rewrite to treat a problem whose evidence points elsewhere. [Evidence Tier: Tier 2 — Strong empirical evidence]
